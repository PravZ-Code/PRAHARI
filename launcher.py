...
# Strictly avoid DETACHED_PROCESS (0x08) which causes Windows conhost to spawn new visible consoles for child processes
SUBPROCESS_FLAGS = CREATE_NO_WINDOW | CREATE_NEW_PROCESS_GROUP

class Win32JobObjectManager:
    """Manages a Windows Job Object to enforce process tree management."""
    def __init__(self, kill_on_close: bool = False):
        ...
        if sys.platform == "win32":
            try:
                ...
                if self.job_handle and self.kill_on_close:
    ...
    def assign_process(self, pid: int):
        """Assign a process to this job object so it is tracked and managed."""
        if not self.job_handle or not pid or pid <= 0:
            return False
        try:
            import ctypes
            kernel32 = ctypes.windll.kernel32
            ...
            if h_proc:
                try:
                    return bool(kernel32.AssignProcessToJobObject(self.job_handle, h_proc))
                finally:
...
GLOBAL_JOB_MANAGER = Win32JobObjectManager(kill_on_close=False)
...
class ServiceManager:
    ...
    def __init__(self, log_callback=None, kill_on_close: bool = False):
        self.log_callback = log_callback or (lambda msg: None)
        self.job_manager = Win32JobObjectManager(kill_on_close=kill_on_close) if kill_on_close else GLOBAL_JOB_MANAGER
        self.procs = {
            "ai": None,
            "backend": None,
            "frontend": None,
    ...
    def start_ai_engine(self):
        ...
        try:
            ...
            if self.procs["ai"] and self.procs["ai"].pid:
                self.job_manager.assign_process(self.procs["ai"].pid)
    ...
    def start_backend(self):
        ...
        # Clear port before launching to prevent address conflicts
        ...
        if not db_file.exists():
            ...
            try:
                py_cli = find_pythonw() or find_python()
                if py_cli:
                    subprocess.run(
        ...
        try:
            ...
            if self.procs["backend"] and self.procs["backend"].pid:
                self.job_manager.assign_process(self.procs["backend"].pid)
    ...
    def start_frontend(self):
        ...
        env["INTERNAL_API_URL"] = "http://127.0.0.1:8000/api"
        ...
        try:
            ...
            if self.procs["frontend"] and self.procs["frontend"].pid:
                self.job_manager.assign_process(self.procs["frontend"].pid)
    ...
    def start_mobile(self, dev_mode=False):
        ...
        if not web_dir.exists():
            web_dir = PRAHARI_DIR / "mobile" / "build" / "web"

        if mobile_server_script.exists():
            cmd = [
        ...
        try:
            ...
            if self.procs["mobile"] and self.procs["mobile"].pid:
                self.job_manager.assign_process(self.procs["mobile"].pid)
...
def run_gui():
    ...
    # Windows Notification Tray Integration (pystray)
    tray_icon = None
    try:
        import pystray
        ...
        def get_tray_image():
            if icon_path and Path(icon_path).exists():
                try:
                    ...
                    pass
            return Image.new('RGB', (64, 64), color=(12, 56, 102))
        ...
        def minimize_to_tray(icon=None, item=None):
            root.after(0, root.withdraw)

        def quit_orchestrator(icon=None, item=None):
            if tray_icon:
                try:
                    tray_icon.stop()
                except Exception:
        ...
        tray_menu = pystray.Menu(
            pystray.MenuItem("Show PRAHARI Dashboard", restore_window, default=True),
            pystray.MenuItem("🌐 Open Web Portal (Port 3000)", lambda: webbrowser.open("http://localhost:3000")),
            pystray.MenuItem("📱 Open Mobile App (Port 8080)", lambda: webbrowser.open("http://localhost:8080")),
            pystray.MenuItem("📄 API Documentation", lambda: webbrowser.open("http://localhost:8000/docs")),
            pystray.Menu.SEPARATOR,
            pystray.MenuItem("▶ Start All Services", lambda: threading.Thread(target=mgr.start_all, daemon=True).start()),
            pystray.MenuItem("■ Stop All Services", lambda: threading.Thread(target=mgr.stop_all, daemon=True).start()),
            pystray.Menu.SEPARATOR,
            pystray.MenuItem("Hide Dashboard", minimize_to_tray),
            pystray.MenuItem("Exit Orchestrator", quit_orchestrator)
        )

        tray_icon = pystray.Icon("PRAHARI", get_tray_image(), "PRAHARI Defense Platform", tray_menu)
        threading.Thread(target=tray_icon.run, daemon=True).start()
    except Exception:
        tray_icon = None

    def handle_hide_tray():
        if tray_icon:
            root.withdraw()
            append_system_log("Dashboard minimized to Windows notification tray. Double-click tray icon to restore.")
        else:
            root.iconify()

    btn_hide_tray.configure(command=handle_hide_tray)
    ...
    # Thread-safe in-memory telemetry state for zero-blocking UI rendering
    class TelemetryState:
        def __init__(self):
            self.lock = threading.Lock()
            self.running = True
            self.service_data = {
                "ai": {"alive": False, "http_status": 0, "latency_ms": 0, "details": "Checking...", "pids": [], "stats": {"cpu_pct": 0.0, "memory_mb": 0.0, "uptime_str": "--:--:--"}},
                "backend": {"alive": False, "http_status": 0, "latency_ms": 0, "details": "Checking...", "pids": [], "stats": {"cpu_pct": 0.0, "memory_mb": 0.0, "uptime_str": "--:--:--"}},
                "frontend": {"alive": False, "http_status": 0, "latency_ms": 0, "details": "Checking...", "pids": [], "stats": {"cpu_pct": 0.0, "memory_mb": 0.0, "uptime_str": "--:--:--"}},
                "mobile": {"alive": False, "http_status": 0, "latency_ms": 0, "details": "Checking...", "pids": [], "stats": {"cpu_pct": 0.0, "memory_mb": 0.0, "uptime_str": "--:--:--"}},
            }
        ...
        def background_poll(self):
            for svc in ["ai", "backend", "frontend", "mobile"]:
                ...
                stats = get_process_resource_stats(probe.get("pids", []))
                probe["stats"] = stats
                with self.lock:
                    self.service_data[svc] = probe
            db_m = get_db_metrics()
            with self.lock:
                self.db_metrics = db_m

        def get_snapshot(self):
            with self.lock:
                return dict(self.service_data), dict(self.db_metrics)

    telemetry_state = TelemetryState()

    def background_telemetry_worker():
        while telemetry_state.running:
            try:
                telemetry_state.background_poll()
            except Exception:
                pass
            time.sleep(1.2)

    telemetry_thread = threading.Thread(target=background_telemetry_worker, daemon=True)
    telemetry_thread.start()

    def refresh_db_bar():
        _, m = telemetry_state.get_snapshot()
        status_text = (
            f"DB [{m.get('status', 'OK')}]: prahari.db ({m.get('size_mb', 0.0)} MB)  |  "
            f"Troopers: {m.get('personnel', 0)}  |  Units: {m.get('units', 0)}  |  "
            f"Grievances: {m.get('grievances', 0)}  |  Users: {m.get('users', 0)}"
        )
        lbl_db_info.configure(text=status_text)

    btn_db_refresh.configure(command=lambda: threading.Thread(target=telemetry_state.background_poll, daemon=True).start())
    ...
    def telemetry_update_loop():
        ...
        # 2. Update Service Health from non-blocking in-memory snapshot (<0.1ms execution)
        svc_snapshot, db_snapshot = telemetry_state.get_snapshot()
        for idx, cfg in enumerate(service_configs):
            svc_key = cfg["key"]
            ui = service_ui[svc_key]
            probe = svc_snapshot.get(svc_key, {})
            stats = probe.get("stats", {"cpu_pct": 0.0, "memory_mb": 0.0, "uptime_str": "--:--:--"})

            if probe.get("alive"):
                status_color = GREEN if probe.get("http_status") in (200, 304, 307) else AMBER
                badge_text = "ONLINE" if probe.get("http_status") in (200, 304, 307) else "DEGRADED"
                ui["dot"].configure(text="●", fg=status_color)
                ui["badge"].configure(text=badge_text, fg=status_color)
                ui["probe"].configure(text=f"HTTP: {probe.get('http_status', 0)} ({probe.get('latency_ms', 0)}ms) · {probe.get('details', '')}", fg="#38bdf8")
                pids_str = ", ".join(str(p) for p in probe.get("pids", [])[:2]) or "--"
                ui["perf"].configure(text=f"PID: {pids_str} | CPU: {stats.get('cpu_pct', 0.0)}% | RAM: {stats.get('memory_mb', 0.0)}MB | Up: {stats.get('uptime_str', '--')}", fg="#e2e8f0")
            else:
        ...
        # 4. Refresh Database Strip from memory snapshot
        status_text = (
            f"DB [{db_snapshot.get('status', 'OK')}]: prahari.db ({db_snapshot.get('size_mb', 0.0)} MB)  |  "
            f"Troopers: {db_snapshot.get('personnel', 0)}  |  Units: {db_snapshot.get('units', 0)}  |  "
            f"Grievances: {db_snapshot.get('grievances', 0)}  |  Users: {db_snapshot.get('users', 0)}"
        )
        lbl_db_info.configure(text=status_text)

        root.after(800, telemetry_update_loop)
    ...
    def on_closing():
        if tray_icon:
            root.withdraw()
            append_system_log("Dashboard minimized to Windows notification tray. Services remain running silently in background.")
        else:
            telemetry_state.running = False
            root.destroy()
...
def main():
    if len(sys.argv) > 1:
        ...
        kill_on_close = (arg in ["--start", "-s", "start"])

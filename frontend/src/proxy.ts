...
export async function proxy(request: NextRequest) {
  ...
  const backendUrl = process.env.INTERNAL_API_URL || process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000/api";
  const backendOrigin = new URL(backendUrl).origin;
  const wsOrigin = backendOrigin.replace(/^http/, "ws");
  response.headers.set(
    "Content-Security-Policy",
    `default-src 'self'; connect-src 'self' ${backendOrigin} ${wsOrigin}; script-src 'self' 'unsafe-inline' 'unsafe-eval'; style-src 'self' 'unsafe-inline'; img-src 'self' data: https:; font-src 'self' data:; frame-ancestors 'none'; base-uri 'self'; form-action 'self'`
  );

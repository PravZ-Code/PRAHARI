  ...
  const renderInlineChipsAndText = (text: string) => {
    ...
    return parts.map((part, i) => {
      ...
      if (citedMatch) {
        ...
        const displayCat = cat.replace(/SHAP_FACTOR/g, "RISK_FACTOR").replace(/SHAP/g, "FACTOR");

        return (
          <span
            key={i}
            title={`Source Check: ${displayCat} | ${desc}`}
          ...
          >
            ...
            <strong className="font-semibold uppercase tracking-wider">{displayCat}:</strong>
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden flex justify-end" role="dialog" aria-modal="true" aria-labelledby="copilot-drawer-title">
      ...
      <div className="relative w-full max-w-2xl bg-slate-900 text-slate-100 border-l border-slate-700/80 shadow-2xl flex flex-col h-full z-10 animate-in slide-in-from-right duration-200">
        ...
        {/* Statutory Non-Clinical Disclaimer Banner */}
        <div className="px-4 py-2 bg-amber-950/40 border-b border-amber-500/30 text-[10px] text-amber-200/90 flex items-center gap-2">
          <Info className="w-3.5 h-3.5 text-amber-400 shrink-0" />
          <span>
            <strong>Decision Support Only:</strong> AI briefings summarize duty rosters and fatigue patterns. Not a clinical psychiatric assessment.
          </span>
        </div>

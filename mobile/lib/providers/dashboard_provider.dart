...
class DashboardProvider with ChangeNotifier {
  ...
  Future<void> loadDashboard() async {
    ...
    try {
      final data = await _apiService.getSafePersonnelStatus();
      _personnelId = data['personnel_id'] ?? '';
      _soldierName = data['name'] ?? 'Trooper';
      _soldierRank = data['rank'] ?? 'Constable';
      _hoursSinceLastDuty = (data['hours_since_last_duty'] as num?)?.toDouble() ?? 9.0;
      _stressPercentage = 0; // Surveillance-free
      _riskCategory = RiskCategory.low;

...
def log_audit(
    ...
    try:
        ...
        # _AUDIT_WRITE_LOCK prevents concurrent threads from reading the same
        # max(sequence_number), and database-level locking/retries handle multi-process concurrency.
        with _AUDIT_WRITE_LOCK:
            max_retries = 3
            for attempt in range(max_retries):
                audit_db = SessionLocal()
                try:
                    if audit_db.bind.dialect.name == "postgresql":
                        from sqlalchemy import text
                        audit_db.execute(text("SELECT pg_advisory_xact_lock(73849102)"))

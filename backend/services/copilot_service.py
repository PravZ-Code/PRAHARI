...
class LocalModelUnavailable(RuntimeError):
    """Raised when a Copilot response cannot be generated by the local model."""

# ---------------------------------------------------------------------------
# 1. CLINICAL LEXICON GUARDRAIL (Mental Healthcare Act 2017 Sec 23 & 115 & Defense Norms)
# ---------------------------------------------------------------------------
...
def sanitize_clinical_lexicon(text: str) -> str:
    ...
    Mental Healthcare Act 2017 (Sec 23 & 115) compliant operational defense terminology.
    """
    if not text:
        return text
    sanitized = text
...
def gather_trooper_dossier(
    ...
    if latest_pred and latest_pred.shap_values:
        raw_shap = latest_pred.shap_values
        ...
        if isinstance(raw_shap, dict):
            for feat, val in raw_shap.items():
                try:
                    impact_val = float(val) if val is not None else 0.0
                except (ValueError, TypeError):
                    impact_val = 0.0
                normalized_shap.append({
                    "feature": str(feat),
                    "impact": impact_val,
                    "value": val
                })
            ...
            for item in raw_shap:
                if isinstance(item, dict):
                    try:
                        impact_val = float(item.get("impact", 0.0))
                    except (ValueError, TypeError):
                        impact_val = 0.0
                    normalized_shap.append({

...
def compute_sync_delta(db: Session, since: Optional[str] = None, unit_id: Optional[str] = None) -> Dict[str, Any]:
    """
    Computes an ultra-fast incremental delta of records modified or created since `since`.
    Uses indexed queries to ensure response times well under 1 millisecond.
    """
    ...
    if unit_id:
        g_query = g_query.join(Personnel, GrievanceRequest.personnel_id == Personnel.id).filter(Personnel.unit_id == unit_id)
    ...
    # 2. Delta Self-Assessments
    a_query = db.query(SelfAssessment).filter(SelfAssessment.assessed_at >= since_dt)
    if unit_id:
        a_query = a_query.join(Personnel, SelfAssessment.personnel_id == Personnel.id).filter(Personnel.unit_id == unit_id)
    ...
    if unit_id:
        b_query = b_query.filter(BuddySignal.unit_id == unit_id)
    ...
    if unit_id:
        w_query = w_query.join(Personnel, WelfareCase.personnel_id == Personnel.id).filter(Personnel.unit_id == unit_id)
        ...
        try:
            with db.begin_nested():
                if "grievance" in action or "request" in action:
                    # Check for existing idempotency key or ID
                    existing = None
                    if queue_id:
                    ...
                    if not existing:
                        ...
                        if not pid and current_user and hasattr(current_user, "personnel_id"):
                            pid = current_user.personnel_id

                        # If still missing, pick first personnel
                        if not pid:
                            p = db.query(Personnel).first()
                            pid = p.id if p else "00000000-0000-0000-0000-000000000001"

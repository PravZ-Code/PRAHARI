...
def _resolve_personnel(db: Session, service_number: Optional[str], phone: Optional[str]) -> Optional[Personnel]:
    ...
    if phone:
        ...
        p = db.query(Personnel).filter(
            (Personnel.contact_number == clean) |
            (Personnel.contact_number.like(f"%{clean[-10:]}"))
        ).first()
        if p:
            return p
    return None

def _get_or_create_unmapped_emergency_personnel(db: Session, caller_phone: Optional[str]) -> str:
    """Provides a designated persistent placeholder for emergency calls from unmapped phone numbers."""
    from models.personnel import Unit
    from datetime import date
    placeholder = db.query(Personnel).filter(Personnel.service_number == "SYS-EMERGENCY-UNMAPPED").first()
    if not placeholder:
        unit = db.query(Unit).first()
        if not unit:
            unit = Unit(
                name="Central Emergency Ingress Unit",
        ...
        placeholder = Personnel(
            service_number="SYS-EMERGENCY-UNMAPPED",
            name="Unregistered Field Caller",
            rank="Trooper",
            trade="Hotline Ingress",
            contact_number=caller_phone or "Unknown",
            unit_id=unit.id,
...
def process_ivr_dtmf(db: Session, req: IVRDTMFRequest) -> IVRDTMFResponse:
    ...
    if digit == "1":
        ...
        target_pid = p_id or _get_or_create_unmapped_emergency_personnel(db, req.caller_phone)
        ...
        case_created_id = _get_or_create_welfare_case(
            ...
            intervention_notes=f"Confidential leave grievance registered via IVR (Caller: {req.caller_phone or 'Unknown'})",
            now=now
        )
        ...
        try:
            ...
            file_grievance_or_leave(
                ...
                description=f"Confidential leave grievance registered via IVR (Caller: {req.caller_phone or 'Unknown'})",
                filing_channel="ivr"
            )
        ...
        from models.personnel import Unit
        target_unit_id = personnel.unit_id if personnel else None
        if not target_unit_id:
            first_unit = db.query(Unit).first()
            target_unit_id = first_unit.id if first_unit else None
        if target_unit_id:
            iso = now.isocalendar()
            sig = BuddySignal(

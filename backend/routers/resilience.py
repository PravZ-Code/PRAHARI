...
class WhatIfTestRequest(BaseModel):
    ...
    night_shifts_removed: Optional[int] = Field(0, ge=0, le=14, description="Night shifts to convert to day/off")
    duty_hours_reduction: Optional[float] = Field(0.0, ge=0.0, le=60.0, description="Weekly duty hours reduction")
    buddy_support_assigned: Optional[bool] = Field(False, description="Assign dedicated trained buddy support pair")
    auto_optimize: Optional[bool] = Field(False, description="Auto-solve minimal intervention bundle to reach GREEN")
...
def post_what_if_duty_test(
    ...
    """
    Flagship Defense What-If Simulation: Runs live counterfactual Calibrated XGBoost inference
    and TreeSHAP waterfall attribution shifts before committing changes to the live roster.
    """
    _verify_resilience_personnel_access(current_user, req.personnel_id, db)
    try:
        return run_what_if_duty_test(
            ...
            night_shifts_removed=req.night_shifts_removed or 0,
            duty_hours_reduction=req.duty_hours_reduction or 0.0,
            buddy_support_assigned=bool(req.buddy_support_assigned),
            auto_optimize=bool(req.auto_optimize)
        )
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))
    except Exception as e:
...
@router.get("/what-if-candidates", summary="List candidate personnel for What-If simulation cockpit")
def get_what_if_candidates(
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user)
):
    """
    Returns personnel for the What-If simulation cockpit with real-time risk levels.
    """
    ...
    elif current_user.role in ("personnel", "jawan") and current_user.personnel_id:
        query = query.filter(Personnel.id == current_user.personnel_id)

    troopers = query.limit(50).all()
    results = []
    for t in troopers:
        pred = db.query(RiskPrediction).filter(
            RiskPrediction.personnel_id == t.id
        ).order_by(RiskPrediction.predicted_at.desc()).first()

        score = float(pred.risk_score) if pred else 0.45
        level = pred.risk_level.upper() if pred else "YELLOW"
        ...
        results.append({
            ...
            "trade": t.trade or "GD",
            "unit_name": t.unit.name if t.unit else "Battalion HQ",

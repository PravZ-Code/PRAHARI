...
def test_mobile_safe_status_privacy_firewall(client: TestClient, personnel_headers: dict):
    """
    CRITICAL STATUTORY PRIVACY TEST:
    Verifies that GET /api/personnel/me/status returns ONLY safe operational guidance.
    Strictly asserts that raw risk scores, SHAP values, and AI risk labels NEVER leak to mobile.
    """
    resp = client.get("/api/personnel/me/status", headers=personnel_headers)
    assert resp.status_code == 200
    data = resp.json()

    # Must contain safe guidance
    assert "status_label" in data
    assert "rest_status" in data
    assert "active_requests_count" in data

    # MUST NEVER contain surveillance or AI scoring keys
    forbidden_keys = ["risk_score", "risk_level", "stress_score", "stress_level", "shap_values", "prediction"]
    for key in forbidden_keys:
        assert key not in data, f"Privacy violation: '{key}' leaked to mobile status endpoint!"

    # Verify content doesn't contain red or alarmist wording
    assert "red" not in str(data.get("status_label", "")).lower()


def test_mobile_token_refresh(client: TestClient, personnel_headers: dict):
    """Verifies POST /api/auth/refresh gives field personnel extended JWT sessions"""
    resp = client.post("/api/auth/refresh", headers=personnel_headers)
    assert resp.status_code == 200
    data = resp.json()
    assert "access_token" in data
...
def test_mobile_idempotent_leave_filing(client: TestClient, personnel_headers: dict):
    """Verifies client-generated UUID idempotency prevents duplicate requests over flaky 2G"""
    import uuid
    ...
    payload = {
        ...
        "description": "Idempotent leave filing test over simulated 2G mobile link.",
    ...
    # 1. First submission
    resp1 = client.post("/api/grievance/", json=payload, headers=personnel_headers)
    assert resp1.status_code in (200, 201)
    req1 = resp1.json()
    assert req1["id"] == client_uuid

    # 2. Duplicate submission (simulating network retry)
    resp2 = client.post("/api/grievance/", json=payload, headers=personnel_headers)
    assert resp2.status_code in (200, 201)
    req2 = resp2.json()
    assert req2["id"] == client_uuid
    assert req2["category"] == req1["category"]


def test_mobile_welfare_endpoints_suite(client: TestClient, personnel_headers: dict):
    """Verifies /api/welfare/checkin, /buddy-signal, /sos, and /api/grievance/mine"""
    headers = personnel_headers

    # 1. GET /api/grievance/mine
    mine_resp = client.get("/api/grievance/mine", headers=headers)
    assert mine_resp.status_code == 200
    assert isinstance(mine_resp.json(), list)

    # 2. POST /api/welfare/checkin
    checkin_resp = client.post("/api/welfare/checkin", json={
        "sleep_quality": "Good",
        "workload_feel": "Manageable",
        "welfare_note": "Well rested after rotation"
    }, headers=headers)
    assert checkin_resp.status_code == 201
    assert "Sent only to your Welfare Officer" in checkin_resp.json()["message"]

    # 3. POST /api/welfare/buddy-signal
    buddy_resp = client.post("/api/welfare/buddy-signal", json={
        "colleague_name": "Constable Amit Singh",
        "concern_type": "Fatigue",
        "note": "Looks tired during night duty"
    }, headers=headers)
    assert buddy_resp.status_code == 201
    assert "Your name is not shared" in buddy_resp.json()["message"]

    # 4. POST /api/welfare/sos
    sos_resp = client.post("/api/welfare/sos", json={
        "message": "Emergency assistance requested via test suite"
    }, headers=headers)
    assert sos_resp.status_code == 201
    assert "case_id" in sos_resp.json()
